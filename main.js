...
// let notActive = users.forEach(function(user) {
//   !user.active ? console.log('Not: ', user.user) : null;
//   return !user.active;
// });
...
//how can we do this with reduce, passing the object as the accumulator?
function getAgeGroups() {
    let result = _.groupBy(peopleData, (person)=>{
...
//with vanilla
// console.log("any perfect scores?", gradeArray.some((grade)=> grade === 100));
...
let checkScore = _.forEach(studentGrades, (item, key)=>{
    // console.log("if statements", item.grade);
...
// console.log("price Groups: ", getPriceGroups() );
...
let needMint = _.forEach(pizzaSpecials, (item, key)=>{
